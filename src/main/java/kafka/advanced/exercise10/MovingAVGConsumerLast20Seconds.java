...
import kafka.advanced.exercise10.commons.exceptions.OutOfOrderException;
import kafka.advanced.exercise10.commons.windowing.Window;
import kafka.advanced.exercise10.commons.windowing.WindowImpl;
...
public class MovingAVGConsumerLast20Seconds {
    ...
    private Map<Location, Map<Window, List<Observation>>> active_windows_tp = new HashMap<>();

    private Map<Location, Long> t0map = new HashMap<>();
    private Map<Location, Long> curr_time = new HashMap<>();
    private Map<Location, Long> last_open = new HashMap<>();
    ...
    private void run() {
        ...
        ConsumerRecords<Location, Observation> poll = consumer.poll(Duration.ofMillis(300));

        assignment.forEach(tp -> {
            List<ConsumerRecord<Location, Observation>> records = poll.records(tp);
        ...
        to_evict.forEach(window -> active_windows_tp.forEach((key, value) -> {
            ...
            List<Observation> remove = value.remove(window);
            if (remove != null)
                remove.clear();
        ...
        while (true) {
            ConsumerRecords<Location, Observation> poll1 = consumer.poll(Duration.ofMillis(300));
    ...
    private void report() {
        active_windows_tp.forEach((key, active_windows) -> {
            ...
            active_windows.keySet().stream()
                    ...
                    .forEach(w -> {
                        System.out.print(" " + w + " ");
                        List<Observation> temperatureValues = active_windows.get(w);
                        temperatureValues.stream().map(Observation::getValue).reduce((t1, t2) -> t1 + t2).ifPresent(integer -> {
    ...
    protected void windowing(Location key, Observation value, Long apptime, long t_e, Long tc0) throws OutOfOrderException {
        ...
        Map<Window, List<Observation>> active_windows = active_windows_tp.get(key);
    ...
    private void scope(Location key, long t_e, long tc0) {
        long o_i;
